...
class Workgroup extends CI_Controller
{
    ...
    public function create()
    {
        if ($this->input->post('txnama') != '') {
            ...
            if ($result->count > 0) {
                ...
                $sql = "INSERT INTO workgroup (workgroupName,  workgroupActive) VALUES ('{$nama}' , 1)";
    ...
    public function edit_table()
    {
        ...
        if ($result > 0) {
            ...
            $res['status'] = 'error';
            $res['msg'] = "workgroup tidak ditemukan";
        }
        echo json_encode($res);
    }
    public function update_table()
    {
        ...
        $this->db->where('workgroupDelete', 0);
        $this->db->where_not_in('workgroupId', $id);
        $query_n = $this->db->get('workgroup');
    ...
    public function active()
    {
        ...
        if ($this->m_workgroup->activate_data($id)) {
            $res["status"] = "success";
